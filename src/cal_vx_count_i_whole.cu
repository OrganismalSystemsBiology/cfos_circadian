...
//nvc++ -mp -ta=tesla cal_vx_count_i_whole.cu -I/opt/nvidia/hpc_sdk/Linux_x86_64/23.3/examples/OpenACC/SDK/include/ -I/tmp/nvhpc_2023_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/include -L/tmp/nvhpc_2023_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/lib -lcudart -std=c++20 -o vx_ci_w
...
__global__ void vx_gpu(int* vxx, int* vxy,  int* vxz, int vx_num, int *x, int *y,  int *z, int *intense, int *vx_count, int cell_count,  int vx, int vb_r){
    ...
    for (int m = 0; m < CT_li.size() ; m += 1) {
        ...
        for (int n = 0; n < sample_ids.size() ; n += 1) {
            ...
            CHECK(cudaMemcpy(d_vx_count, vx_count, sizeof(int)*vx_num, cudaMemcpyHostToDevice));   
                ...
                vx_gpu<<<grid, block>>>(d_vxx, d_vxy,  d_vxz, vx_num, d_x, d_y,  d_z, d_intense, d_vx_count,  cell_count,  vx, vb_r);
