...
//nvc++ -mp -ta=tesla cal_vx_count_i.cu -I/opt/nvidia/hpc_sdk/Linux_x86_64/23.3/examples/OpenACC/SDK/include/ -I/tmp/nvhpc_2023_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/include -L/tmp/nvhpc_2023_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/lib -lcudart -std=c++20 -o vx_ci
//./vx_ci 2 2 2 144 197 175  SCH 50 3 1 0.5 /home/gpu_data/data8/cfos_app ANTsR50 2 2 2
//./vx_ci 153 163 82 114 165 157 SCH 50 3 1 0.5 /home/gpu_data/data8/cfos_app ANTsR50 cell_intensity_norm.npy 2 8 8 8
...
__global__ void vx_gpu(int *x, int *y,  int *z, int *intense, int *vx_count, int cell_count, int x_b_num, int y_b_num, int z_b_num, int xmin, int ymin, int zmin,  int vx, int vb_r, int mo){
        ...
        for (int n = 0; n < sample_ids.size() ; n += 1) {
            ...
            if (fileExists(vx_f)) {
                std::cout << vx_f << " exists" << std::endl;
                        ...
                        std::string cell_npy = savedir +"/"+ exp+"/" +  sample +"/cfos/"+ants_dir_name +"/"+combine_points_f;
                        ...
                        CHECK(cudaMemcpy(d_vx_count, vx_count, sizeof(int)*vx_num, cudaMemcpyHostToDevice));   
                            ...
                            vx_gpu<<<grid, block>>>(d_x, d_y,  d_z, d_intense, d_vx_count,  cell_count, x_b_num, y_b_num, z_b_num, xmin, ymin, zmin,  vx, vb_r, mo);
