...
//nvc++ -mp -ta=tesla cal_vx_count_pro_i.cu -I/opt/nvidia/hpc_sdk/Linux_x86_64/23.3/examples/OpenACC/SDK/include/ -I/tmp/nvhpc_2023_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/include -L/tmp/nvhpc_2023_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/lib -lcudart -std=c++20 -o vx_pro_ci
//./vx_pro_ci 2 2 2 144 197 175  SCH 50 3 1 0.5 /home/gpu_data/data8/cfos_app ANTsR50 2 2 2
//./vx_pro_ci 153 163 82 114 165 157 SCH 50 3 1 0.5 /home/gpu_data/data8/cfos_app ANTsR50 cell_intensity_norm.npy 2 8 8 8
...
__global__ void vx_gpu_hor(int *x, int *y,  int *z, int *intense, int *vx_count, int cell_count, int x_b_num, int y_b_num, int z_b_num, int xmin, int ymin, int zmin, int xmax, int ymax, int zmax,  int vx, int vb_r, int mox, int moy, int moz){
     ...
     if (iy < y_b_num) {
        if (iz < z_b_num) {
                ...
                for (int it=0; it<cell_count; it++){
                    ...
                    // }
                    if((x[it]>=xmin*vx+ix*mox*vx-int(vb_r*vx/2))&(x[it]<xmin*vx+ix*mox*vx+int(vb_r*vx/2))&(y[it]>=ymin*vx+iy*moy*vx-int(vb_r*vx/2))&(y[it]<ymin*vx+iy*moy*vx+int(vb_r*vx/2))&(z[it]>=zmin*vx)&(z[it]<zmax*vx)){
                    sum+=intense[it];
                      }//if(x~)
                    } //for  cell_count
                vx_count[idx] = sum;
...
__global__ void vx_gpu_cor(int *x, int *y,  int *z, int *intense, int *vx_count, int cell_count, int x_b_num, int y_b_num, int z_b_num, int xmin, int ymin, int zmin, int xmax, int ymax, int zmax,  int vx, int vb_r, int mox, int moy, int moz){
...
__global__ void vx_gpu_sag(int *x, int *y,  int *z, int *intense, int *vx_count, int cell_count, int x_b_num, int y_b_num, int z_b_num, int xmin, int ymin, int zmin, int xmax, int ymax, int zmax,  int vx, int vb_r, int mox, int moy, int moz){
    ...
    for (int m = 0; m < CT_li.size() ; m += 1) {
        ...
        for (int n = 0; n < sample_ids.size() ; n += 1) {
            ...
            for (int i = 0; i < cell_count ; i += 1) {
                ...
                intense[i] = static_cast<int>(cell_cords[j*cell_count+i]);
                }
            ...
            // printf("aa\n");
            //device memory
            int *d_x,  *d_y, *d_z, *d_intense, *d_vx_count;
            ...
            CHECK(cudaMemcpy(d_vx_count, vx_count, sizeof(int)*vx_num, cudaMemcpyHostToDevice));   
                ...
                vx_gpu_hor<<<grid, block>>>(d_x, d_y,  d_z, d_intense, d_vx_count,  cell_count, x_b_num, y_b_num, z_b_num, xmin, ymin, zmin, xmax, ymax, zmax,  vx, vb_r, mox, moy, moz);
                }else if (angle == "cor"){
                    int dimx =x_b_num/blockdim_x;// (NE+NI)/blockdim_x;
                    int dimy = y_b_num/1;
                    int dimz = z_b_num/blockdim_z;
                    ...
                    dim3 grid(dimx, dimy, dimz);
                    vx_gpu_cor<<<grid, block>>>(d_x, d_y,  d_z, d_intense, d_vx_count,  cell_count, x_b_num, y_b_num, z_b_num, xmin, ymin, zmin, xmax, ymax, zmax,  vx, vb_r, mox, moy, moz);
                }else if (angle == "sag"){
                    ...
                    dim3 grid(dimx, dimy, dimz);
                    vx_gpu_sag<<<grid, block>>>(d_x, d_y,  d_z, d_intense, d_vx_count,  cell_count, x_b_num, y_b_num, z_b_num, xmin, ymin, zmin, xmax, ymax, zmax,  vx, vb_r, mox, moy, moz);
                }
