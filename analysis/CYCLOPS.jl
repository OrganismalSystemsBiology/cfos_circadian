...
const mouse_acrophases = [4.84994781356335, 2.7598866971826475, 1.1350979035153448, 5.9432920883469045, 1.28745074367518, 1.4082924770872276, 2.5601201378513196, 2.5616107041149814, 5.884697947265382, 1.3809497132140345, 4.726492099602786, 2.560242692208792, 3.7528521994796913, 2.6431494997494185, 5.383122416506001]
const mouse_gene_symbol = ["SCH","IAD","AUDp5","SO","AUDp6a","AUDpo6a","ND","IPR","opt","AUDpo5","PVa","AON","RSPv2/3","SSp-bfd2/3","RM"]
...
const human_homologue_gene_symbol = mouse_gene_symbol#uppercase.(mouse_gene_symbol)
const subfolders = ["Plots", "Fits", "Models", "Parameters"]

# base_path = joinpath(pwd(), "", "")
# output_path = joinpath(base_path, "results")
...
function DefaultDict()
	...
	:seed_min_CV => 0.85,# 0.14, 							# The minimum coefficient of variation a gene of interest may have to be included in eigen gene transformation
	:seed_max_CV => 0.1,#0.9, 							# The maximum coefficient of a variation a gene of interest may have to be included in eigen gene transformation
	:seed_mth_Gene => 3,#197, 						# The minimum mean a gene of interest may have to be included in eigen gene transformation
	...
	:eigen_first_var_cutoff => 5.0,#0.85, 				# Cutoff used on captured variance of first eigen gene

	:eigen_total_var => 5.0, #0.85, 						# Minimum amount of variance required to be captured by included dimensions of eigen gene data
	:eigen_contr_var => 0.01, #0.05, 						# Minimum amount of variance required to be captured by a single dimension of eigen gene data
	:eigen_var_override => true,					# Is the minimum amount of contributed variance ignored
	:eigen_max => 99,#5, 								# Maximum number of dimensions allowed to be kept in eigen gene data
	...
	:train_collection_time_balance => 1.0, #0.1,					# How is the true time loss rescaled
...
function generate_short_path(long_path::String, max_length::Int64)
    if length(long_path) > max_length
        return long_path[1:max_length] * "..."
    else
        return long_path
    end
end
...
function OutputFolders(output_path, ops)
	println("\tCREATING OUTPUT FOLDER\n\n")
	print(output_path)
	# output_path = generate_short_path(output_path)
	print(output_path)
	...
	master_output_folder_path = joinpath(output_path, path_extentsion)
	# master_output_folder_path = generate_short_path(master_output_folder_path)
